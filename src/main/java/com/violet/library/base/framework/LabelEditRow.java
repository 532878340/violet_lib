...
/**
 * 通用的显示label和文本的View
 * Created by Perley on 2015/4/30.
 */
public class LabelEditRow extends LabelTextRow {
    ...
    protected void bindFromAttrs(AttributeSet attrs) {
        ...
        if (!TextUtils.isEmpty(text)) {
            label.setText(text);
            // 有设置文本时，保持最小宽度
            label.setMinWidth(getResources().getDimensionPixelSize(R.dimen.label_edit_min_width));
        }
        ...
        if(scaleLeft != INVALID){
            ...
            final int size = PhoneUtils.dpToPx(getContext(), 20);
        ...
        // 字体大小
        float textSize = styled.getDimensionPixelSize(R.styleable.LabelTextRow_labelTextSize, INVALID);
        if(textSize != INVALID) {
            label.setTextSize(TypedValue.COMPLEX_UNIT_PX,textSize);
            value.setTextSize(TypedValue.COMPLEX_UNIT_PX,textSize);
            unit.setTextSize(TypedValue.COMPLEX_UNIT_PX,textSize);
        }

        // EditText字体大小
        textSize = styled.getDimensionPixelSize(R.styleable.LabelTextRow_labelValueSize, INVALID);
        if(textSize != INVALID){
            value.setTextSize(TypedValue.COMPLEX_UNIT_PX,textSize);
        }
